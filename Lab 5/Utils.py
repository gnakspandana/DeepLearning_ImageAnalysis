...
# Function for loading a smaller part of the dataset (Lab 5's dataset is so large the kernel crashes)
def load_partial_data(data_dir, image_dir, mask_dir, img_width, img_height, number_of_images=0, partition=None):
    
    # Choose either number of images (by specifying number_of_images) or percentage of dataset (0 < partition < 1) when calling the function. If neither is specified then all data will be loaded. 
    if number_of_images==0:
        number_of_images=len(os.listdir(image_dir))
    
    if isinstance(partition, float):
        number_of_images = floor(len(os.listdir(image_dir))*partition)
    
    image_paths = [os.path.join(image_dir, img) for img in sorted(os.listdir(image_dir))[0:number_of_images]]
    mask_paths = [os.path.join(mask_dir, mask) for mask in sorted(os.listdir(mask_dir))[0:number_of_images]]
    
    # Debugging code to make sure the image and the labels are matched
    print("load_partial_data: Printout to see that the images and labels are matched:")
...
def binarize_mask(data, img_height, img_width, val_lower=0.5, val_upper=1):
    for i in range(0,len(data)):
        for j in range(0,img_height):
            for k in range(0,img_width):
                if data[i][j][k] > val_lower and data[i][j][k] < val_upper:
                    data[i][j][k] = 1
                else:
                    data[i][j][k] = 0
